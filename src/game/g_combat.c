...
void player_die( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int meansOfDeath )
{
  ...
  if( self->client->ps.pm_type == PM_DEAD )
	...
	if( attacker != self && attacker->client && self->client && attacker->client->ps.stats[ STAT_PTEAM ]  == self->client->ps.stats[ STAT_PTEAM ] )
	{
         ...
         vec3_t dir, vec3_t point, int damage, int dflags, int mod )
{
  ...
  if( targ->client )
  {
    ...
    {
      damage = 1;//1 dmg to self
    }
    if(mod == MOD_FLAMER && targ != attacker)
    {
      targ->client->ps.stats[STAT_STATE] |= SS_SLOWLOCKED;
      targ->client->lastSlowTime = level.time + 3000;
    }
    knockback = (int)( (float)knockback *
