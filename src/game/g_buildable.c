...
static gentity_t *G_Build( gentity_t *builder, buildable_t buildable, vec3_t origin, vec3_t angles )
{
  ...
  if( builder && builder->client ) // log ingame building only
  {
	...
	if( builder->client ) {
		...
		{
			G_TeamCommand( PTE_HUMANS, va("cp \"^5Reactor^7 built by %s\n^7Buildlog #%i\" 1",builder->client->pers.netname,new->ID));
			G_TeamCommand( PTE_HUMANS, va("print \"^7Reactor built by %s^7. Buildlog #%i\" 1",builder->client->pers.netname,new->ID));

		}
		else if(buildable == BA_A_OVERMIND)
		{
			G_TeamCommand( PTE_ALIENS, va("cp \"^1Overmind^7 built by %s\nBuildlog #%i\" 1",builder->client->pers.netname,new->ID));
			G_TeamCommand( PTE_ALIENS, va("print \"^7Overmind built by %s^7.Buildlog #%i\" 1",builder->client->pers.netname,new->ID));
